...
AnalysisStyle = 'Separation';  % optional: 'Integration' and 'Seperation'
GroupName = {'Littermates','SNCA'};  % adjust groups as needed
% GroupName = {'Monomer injection','PFF injection','PFF injection activation'};  % adjust groups as needed
filesavepath = 'D:\Data\2021_NoLaserODPAperformance_SncaLitter\Training';  % path to save results
IsPseudoDelay = 0;
...
ColorSet = {[0 0 0],[0 125 0]/255};  % group colors
...
TickLabelSize = 16;
AxisLabelSize = 16;
TimesID = 1:1:TimesNum;
TimesNum = 5; % number of learning days
TrialNuminWindow = [];
...
if strcmp(AnalysisStyle, 'Separation')
    ...
    for iDelay = 1:numel(DelayLen)
        figure('Position',[219 303 750 600]);
        CompareDiffGroupsTimeLapsedBehavior(phase, 'HitRate', ['delay' num2str(iDelay)], AllSessionMiceBehavior, ...
            ColorSet, LineWidth, LineStyle, MarkerShape, ColorSet, ColorSet, MarkerSize, filesavepath);
        CompareDiffGroupsTimeLapsedBehavior(phase, 'CRRate', ['delay' num2str(iDelay)], AllSessionMiceBehavior, ...
            ColorSet, LineWidth, LineStyle, MarkerShape, ColorSet, ColorSet, MarkerSize, filesavepath);
        legend(GroupName,'Location','southeast');
        legend('boxoff');
        % Set the coordinate axes
...
if strcmp(AnalysisStyle, 'Integration')
    for iDelay = 1:numel(DelayLen)
        ...
        % plot bar graphs of licking rates on hit trials
