function metrics = CalculatePerfAfterRemovingUnmotivatedTrials(metrics,TrialStructure, SetTrialsNum, SetWindowNum, Threshold)
    ...
    for i = 1:num_windows
        window_counts(i) = struct('Hits', 0, 'Misses', 0, 'FAs', 0, 'CRs', 0);
    end
    ...
    for i = 1:num_windows
        ...
        total_go_signals = window_counts(i).Hits + window_counts(i).Misses;
        if total_go_signals > 0
            HitRateinWindow(i) = 100 * window_counts(i).Hits / total_go_signals;
            MissRateinWindow(i) = 100 * window_counts(i).Misses / total_go_signals;
        end
        
        total_nogo_signals = window_counts(i).FAs + window_counts(i).CRs;
        if total_nogo_signals > 0
            FalseRateinWindow(i) = 100 * window_counts(i).FAs / total_nogo_signals;
            CRRateinWindow(i) = 100 * window_counts(i).CRs / total_nogo_signals;
        end
    ...
    if isempty(metrics.NewTrialStructure)
        ...
        total_go_signals_final = hits_final + misses_final;
        total_nogo_signals_final = fas_final + crs_final;
        total_trials_final = size(metrics.NewTrialStructure, 1);

        HitRateinSession = 100 * hits_final / total_go_signals_final;
        MissRateinSession = 100 * misses_final / total_go_signals_final;
        FalseRateinSession = 100 * fas_final / total_nogo_signals_final;
        CRRateinSession = 100 * crs_final / total_nogo_signals_final;
