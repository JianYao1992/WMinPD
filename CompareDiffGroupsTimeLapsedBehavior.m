function CompareDiffGroupsTimeLapsedBehavior(Phase, BehaviorMeasure, DelayLength, Data, ...
...
requiredParams = {Phase, BehaviorMeasure, DelayLength, Data, SavePath};
paramNames = {'phase', 'behaviormeasure', 'delaylength', 'data', 'savepath'};
for i = 1:numel(requiredParams)
    if isempty(requiredParams{i})
        error('%s cannot be empty!', paramNames{i});
    end
end
...
for groupIdx = 1:numGroups
    ...
    groupData = Data{groupIdx}.(DelayLength).(BehaviorMeasure);
    % Plot data
...
filePrefix = fullfile(SavePath, sprintf('%s comparison-Separation-%s trials-%s', ...
    BehaviorMeasure, DelayLength, Phase));
...
try % Exception handling to prevent program crash caused by single-step errors
    if numel(TargetData{1}) > 1
        ...
        if numel(TargetData) < 3
            pValue = GetDataMatrixForMixedRepeatedAnova(TargetData, filePrefix);
        else
            TwMDanovaWithPostHoc(TargetData, filePrefix);
        end
    else
        ...
        % Save p-value
        saveFile = fullfile(SavePath, sprintf('Pvalue for %s comparison_Separation_%s_%s.mat', ...
            BehaviorMeasure, DelayLength, Phase));
