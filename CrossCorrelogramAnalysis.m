...
updateUnitLabels(Units, spikeTrials, TimeGain, BaselineLen, SampleOdorLen, DelayLen, TestOdorLen, RespWindowLen, 'No laser');
...
updateUnitLabels(Units, spikeTrials_off, TimeGain, BaselineLen, SampleOdorLen, DelayLen, TestOdorLen, RespWindowLen, 'Laser off');
updateUnitLabels(Units, spikeTrials_on, TimeGain, BaselineLen, SampleOdorLen, DelayLen, TestOdorLen, RespWindowLen, 'Laser on');
...
function updateUnitLabels(Units, spikeTrials, TimeGain, BaselineLen, SampleOdorLen, DelayLen, TestOdorLen, RespWindLen, trial_type)
for iUnit = 1:size(Units, 1)
    % Initialize label fields
    spikeTrials.label{iUnit,2} = []; % Waveform stats
    spikeTrials.label{iUnit,3} = []; % Waveform voltage
    spikeTrials.label{iUnit,7} = Units{iUnit,3}; % Brain region

    % Sample preference
    time_window = BaselineLen : BaselineLen + SampleOdorLen + DelayLen + TestOdorLen;
    switch trial_type
        case 'No laser'
            spikeTrials.label{iUnit,4} = Units{iUnit,24}(time_window);

        case 'Laser off'
            spikeTrials.label{iUnit,4} = Units{iUnit,24}.laseroff(time_window);

        case 'Laser on'
            spikeTrials.label{iUnit,4} = Units{iUnit,24}.laseron(time_window);
    end
    ...
    spikeTrials.label{iUnit,5} = s1_fr; % S1 FR
    spikeTrials.label{iUnit,6} = s2_fr; % S2 FR
end
end
