...
function updateUnitLabels(Units, result, TimeGain, BaselineLen, SampleOdorLen, DelayLen, TestOdorLen, RespWindLen, trial_type)
for iUnit = 1:size(Units, 1)
    % Initialize label fields
    result.label{iUnit,2} = []; % Waveform stats
    result.label{iUnit,3} = []; % Waveform voltage
    result.label{iUnit,7} = Units{iUnit,3}; % Brain region
    % Sample preference
    ...
    switch trial_type
        case 'No laser'
            result.label{iUnit,4} = Units{iUnit,24}(time_window);
            result.label{iUnit,5} = s1_fr; % S1 FR
            result.label{iUnit,6} = s2_fr; % S2 FR
        case 'Laser off'
            result.label{iUnit,4}.laseroff = Units{iUnit,24}.laseroff(time_window);
            result.label{iUnit,5}.laseroff = s1_fr; % S1 FR
            result.label{iUnit,6}.laseroff = s2_fr; % S2 FR
        case 'Laser on'
            result.label{iUnit,4}.laseron = Units{iUnit,24}.laseron(time_window);
            result.label{iUnit,5}.laseron = s1_fr; % S1 FR
            result.label{iUnit,6}.laseron = s2_fr; % S2 FR
    end
