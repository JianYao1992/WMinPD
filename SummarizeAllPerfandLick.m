function AllSessionMiceBehavior = SummarizeAllPerfandLick(Data, SummaryMode, TimeID, TimesNumber, LickEfficiencyAnaRange, TrialNuminEachWindow, DelayLength, TimeGain, Category)
...
function lickRates = calculateLickRates(LickNumber, TrialOutcome, TimeGain, Constants)
...
if nnz(maskGo) > 0
    lickRates.LickRate_Go = TimeGain * mean(LickNumber(maskGo, :), 1, 'omitnan');
else
    lickRates.LickRate_Go = NaN;
end
if nnz(maskHit) > 0
    lickRates.LickRate_Hit = TimeGain * mean(LickNumber(maskHit, :), 1, 'omitnan');
else
    lickRates.LickRate_Hit = NaN;
end
if nnz(maskNoGo) > 0
    lickRates.LickRate_NoGo = TimeGain * mean(LickNumber(maskNoGo, :), 1, 'omitnan');
else
    lickRates.LickRate_NoGo = NaN;
end
if nnz(maskFalse) > 0
    lickRates.LickRate_False = TimeGain * mean(LickNumber(maskFalse, :), 1, 'omitnan');
else
    lickRates.LickRate_False = NaN;
end
end
