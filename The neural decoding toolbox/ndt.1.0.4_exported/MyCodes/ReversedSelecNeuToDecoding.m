...
% TrialTypeAndLaserEffecsOnDecoding
clear;clc;close all;
LaserEffectOnDecodingFiles=dir('*LaserEffectsOnDecoding*.mat');
Legends=[{'WithReversedSelec'};{'WithouRevSelec'}];
Colors=[[0 0 0];[0 0 1];[1 0 0]];

DecodingTimesToCompare=20;
load(LaserEffectOnDecodingFiles(1).name,'AllPeriodsDiffMeanDecodingAccuracyChRNL')
...
for iGroup=1:2
    ...
    for iPeriod=2%1:length(AllPeriodsDiffDecodingAccuracyNpHRNL)
        ...
        bar(1:length(Mean),Mean,'facecolor','none','edgecolor',[0 0 1])
        hold on
        errorbar(1:length(Mean),Mean,STD,'LineStyle','none','linewidth',2)%,'linestype','none'
        ...
        set(gca,'xtick',1:length(Legends),'xticklabel',Legends)
        ylabel('Diff mean decoding accuracy','fontsize',12)
        %axis([0.5 3.5 Min-1 Max*1.5])
        title([tempGroupID '-' TargetDelayMarker],'fontsize',16)
        saveas(gcf,[tempGroupID '-ReversedNeu and laser effects on decoding-' TargetDelayMarker '-' num2str(DecodingTimesToCompare)],'fig')%
        saveas(gcf,[tempGroupID '-ReversedNeu and laser effects on decoding-' TargetDelayMarker '-' num2str(DecodingTimesToCompare)],'png')%
        close all
    end
end