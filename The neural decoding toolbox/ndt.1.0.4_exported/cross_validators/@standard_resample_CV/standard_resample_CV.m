...
%           cross-validation split.  num_resample_runs x num_CV_splits x TrainBinNumber x testBinNumber
...
%==========================================================================   
    ...
    methods        
        ...
        function DECODING_RESULTS = run_cv_decoding(cv) %cv=the_cross_validator;
            ...
            while ~(minimum_resample_runs_executed_and_results_have_converged)%go through each resample run % iResample = 1:cv.num_resample_runs
                ...
                [all_XTr, all_YTr, all_XTe, all_YTe] = datasource.get_data;  %get_data
                 %all_XTr£ºone cell for each time bine
                 %all_XTr{i}£ºone cell for each cross-validation split within bin(i)
                 %all_XTr{i}{j}£ºNeuron_Num X ((num_cv_splits-1) X Stimulus_Num X num_times_to_repeat_each_label_per_cv_split)
                ...
                for iCV = 1:size(all_XTr{1}, 2) %go through each cross-validation

                    for iTrainingInterval = 1:size(all_XTr, 2)%go through each time bin
                            ...
                            if ~isempty(cv.feature_preprocessors)
                                for iFP = 1:length(cv.feature_preprocessors)
                                    %set_properties_with_training_data                                    
                                    ...
                                    %normalize to z-score for specific bin without distinguish the stimulus identity add by CQ              
                            ...
                            classifier = classifier.train(XTr, YTr);%train                            
                            ...
                            for iTestInterval = test_interval    
                                ...
                                %normalize test data to Z-score with the mean and std
                                %of the train data, and did not distingush the stimulus identity add by CQ
                                if ~isempty(cv.feature_preprocessors)
                                    for iFP = 1:length(cv.feature_preprocessors)       
                                        XTe = feature_preprocessors{iFP}.preprocess_test_data(XTe);%preprocess_test_data                     
                                    end
                                end

                                % test the classifier
                                [predicted_labels,decision_values] = classifier.test(XTe); %test  classifier
                                ...
                                DECODING_RESULTS.ZERO_ONE_LOSS_RESULTS.decoding_results(iResample, iCV, iTrainingInterval, iTestIntervalSaveInd, :) = ...
                                    ((length(find(predicted_labels - YTe == 0))/length(predicted_labels)));  
                                   %corrected predicted trial number/condition number
                ...
                % normalizing the rank confusion matrix so that each column (actual label) contains the real normalized rank values (i.e., vtotal_rank_cmalues in the range [0 1])
               ...
               if iResample > 1 
                 ...
                 LampsedTime=toc;
                 disp(['---ReSample Run-' num2str(iResample) '-' num2str(floor(LampsedTime)) '---'])
