...
%
%    a. XTr_all_time_cv{iTime}{iCV} = [num_features x num_training_points]
%    is a   cell array that has the training data for all times and cross-validation splits
                                     %//Neuron_Num x ((num_cv_splits-1) x Stimulus_Num)//add by CQ                 
...
methods
        ...
        function ds = basic_DS(binned_data_name, specific_binned_label_name, num_cv_splits, load_data_as_spike_counts)
            ...
            if load_data_as_spike_counts > 0
                ...
                [binned_data_spike_counts, binned_site_info] = load_binned_data_and_convert_firing_rates_to_spike_counts(binned_data_name);
 ...
 %%
        ...
        function  [XTr_all_time_cv, YTr_all, XTe_all_time_cv, YTe_all] = get_data(ds) %ds=datasource;
        ...
        %         
           ...
           if  create_simultaneously_recorded_populations == 0                      
               ...
               for iLabel = label_names_to_use%go through each condition

                    cNeuron = 1;    
                    for iNeuron = unique(curr_resample_sites_to_use)  %Go through each neuron
           ...
           clear the_data   % clear up some memory            
            ...
            for iTimePeriod = 1:length(time_periods_to_get_data_from)%go through each time bin        
                ...
                cv_start_ind = 1;                
                for iCV = 1:num_cv_splits%go through each cross-validation split                   
                    curr_cv_inds = [];  %NaN .* ones(num_times_to_repeat_each_label_per_cv_split * length(ds.label_names_to_use), 1);                    
                    %pick the trial index for test data add by CQ
