classdef plot_standard_results_TCT_object
    ...
    methods
        ...
        function min_and_max_data_range = plot_results(plot_obj) 
            ...
            %  then create the x-axis time interval from these parameters, 
            %  or if the_time_interval  is empty and DS_PARAMETERS.binned_site_info.binning_parameters exist,
            %  use these parameters to create a time interval.  
            %  Also make sure that plot_time_intervals has not been set to a vector of times.
...
%             end
            ...
            if isempty(the_time_interval)  % otherwise if binning_parmaters don't exist (and no interval has been set) just use consecuative numbers on x-axis
                the_time_interval = 1:length(result_to_plot);
            end            
            ...
            if ~isempty(plot_obj.color_result_range)
                imagesc(the_time_interval,the_time_interval,result_to_plot, plot_obj.color_result_range);
            else
                imagesc(the_time_interval,the_time_interval,result_to_plot);
            end
...
%             end
            ...
            set(gca,'xtick',[0 1 2 3 4 5 10], 'XTickLabel', [0 1 2 3 4 5 10],'ytick',[0 1 2 3 4 5 10], 'yTickLabel', [0 1 2 3 4 5 10]);%add by CQ     
            ...
            for iEvent = 1:length(plot_obj.significant_event_times)
                ...
                line(get(gca, 'XLim'), [plot_obj.significant_event_times(iEvent), plot_obj.significant_event_times(iEvent)], 'color', [0 0 0])                
            end
