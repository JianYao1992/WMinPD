classdef plot_standard_results_object
    ...
    %
    %   p_values.  If this is set to a cell array of numbers that is the same length as result_file_names that vectors that of p-values
    %       for each time point, then bars will be shown at the bottom of the plot for all times that the p_values are less than
    %       p_value_alpha_level.  If this is set to a cell array of strings that contain the names of directories that have files
    %       that comprise a null distribution, the p-values will be calculated and a bars will be shown at the  bottom of the plot for
    %       all times that the p_values are less than p_value_alpha_level. The width of the interval indicating whether a data point
    %       is above chance is the time when a data point is above chance +/- 1/2 of the bin width.  For more information about how
    %       to run decoding analyses to estimate p-values, see the file tools/create_pvalues_from_null_distribution_files.m).
    %
    ...
    %
    %   add_pvalue_latency_to_legends_alignment (default = 4).  If this property is set to value greater than 0, and if the p_values field
    %   has been set, then the latency when the decoding results are above chance will be added to the legend.  If this value is set to 1 it
    %   will show the beginning of the time interval when the results are above chance, if it is set to 2 it will show the middle of the time 
    %   interval when the results are above chance, if this is set to 3 it will show the end of the time interval when the results are above
    %   chance and if this is set to 4 (default) it will show the beginning and the end of the interval when the results are above chance. 
    %   If this property is set to 5, then the time interval that was added to make the results above chance will be displayed (i.e., the time
    %   of the first interval that is above chance will be displayed minus any time that is in the previous interval (which was the
        ...
        function plot_results(plot_obj)
            ...
            for iResult = 1:number_of_results_to_plot
                ...
                if iscell(plot_obj.result_file_names)
                    ...
                    if size(curr_result_to_plot, 2) ~= 1% temporal cross training(TCT) Time lampsed decoding
            ...
            %add by CQ
            YMin=min(curr_result_to_plot(curr_plot_inds));
            %%
            ...
            if ~isempty(plot_obj.errorbar_file_names)
                ...
                for iErrorBar = 1:number_of_errorbars
                    ...
                    fill([all_time_intervals{iErrorBar}(all_plot_inds{iErrorBar}) fliplr(all_time_intervals{iErrorBar}(all_plot_inds{iErrorBar}))],...
                        [upper_errorbar(all_plot_inds{iErrorBar}) fliplr(lower_errorbar(all_plot_inds{iErrorBar}))], plot_obj.the_colors{iErrorBar}*0.8,...
                        'EdgeColor', plot_obj.the_colors{iErrorBar}*0.8, 'EdgeAlpha', plot_obj.errorbar_edge_transparency_level, 'FaceAlpha', plot_obj.errorbar_transparency_level)
                    
                    YMin=min(lower_errorbar);
                    YMax=max(upper_errorbar);
                end
            end   % end for plotted the errorbars
            
            %%
            %replot decoding accuracy add by CQ
            plot(curr_time_interval(curr_plot_inds), curr_result_to_plot(curr_plot_inds), plot_obj.line_style, 'LineWidth', plot_obj.line_width, 'Color', plot_obj.the_colors{iResult});
            ...
            if ~isempty(plot_obj.p_values)
                ...
                for iResult = 1:number_of_results_to_plot
                    ...
                    if ischar(plot_obj.p_values{iResult}) %if isstr(plot_obj.p_values{iResult})
                        ...
                        plot_obj.p_values{iResult} = pval_obj.create_pvalues_from_nulldist_files; %create_pvalues_from_nulldist_files
                    ...
                    %permutation test add by CQ
                    % The length of p-value significance markers are based on the distance between adjacent sampled points
                    %(i.e., the sampling_interval).  The length of each marker is from the center of the bin to 1/2 the sampling_interval 
                    %to the left and 1/2 the sampling_interval to the right.
                    ...
                    for iSigInterval = 1:length(inds_to_use)
                        line([start_intervals(inds_to_use(iSigInterval)) end_intervals(inds_to_use(iSigInterval))], [40 40], 'color', plot_obj.the_colors{iResult}, 'LineWidth', 5)
                    end%[y_offset y_offset] YMax
            ...
            if ~isnan(plot_obj.chance_level)
                line(get(gca, 'XLim'), [plot_obj.chance_level plot_obj.chance_level], 'color', [0 0 0],'linewidth',2)
            end
            ...
            for iEvent = 1:length(plot_obj.significant_event_times)
                ...
                plot([plot_obj.significant_event_times(iEvent), plot_obj.significant_event_times(iEvent)], [0 100],'k--','linewidth',1.5)
            end
            ...
            box off
            xlim([min(curr_time_interval) max(curr_time_interval(curr_plot_inds))])
            ylim([YMin*0.9 100])
            %set(gca,'xtick',[20-5 70-5 120-5],'xticklabel',[0 5 10],'ytick',[20-5 70-5 120-5],'yticklabel',[0 5 10], 'tickdir', 'out')
