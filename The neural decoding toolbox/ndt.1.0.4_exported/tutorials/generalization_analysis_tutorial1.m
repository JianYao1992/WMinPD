...
toolbox_dir = 'D:\CQ\Matlab codes\Population Analysis\The neural decoding toolbox\ndt.1.0.4_exported/';
...
binned_data_file_name = create_binned_data_from_raster_data(raster_file_directory_name,...
    save_prefix_name, bin_width, step_size, start_time, end_time);
%%  3.  Create a classifier and a feature proprocessor object
the_classifier = max_correlation_coefficient_CL;
the_feature_preprocessors{1} = zscore_normalize_FP;  
%%  4. Let's first train the classifier to discriminate between objects at the upper location,
% and test the classifier with objects shown at the lower location

%%  4a.  create labels for which exact stimuli (ID plus position) belong in the training set,
% and which stimuli belong in the test set
...
for iTrainPosition = 1:3%go through each location
   ...
   for iTestPosition = 1:3
      ...
      % save the results
      save_file_name = ['position_invariance_results/Zhang_Desimone_pos_inv_results_train_pos' num2str(iTrainPosition)...
          '_test_pos' num2str(iTestPosition)]; 
