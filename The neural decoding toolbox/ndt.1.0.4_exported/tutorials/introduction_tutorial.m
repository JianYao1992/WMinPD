...
PlotFigure=0;
toolbox_directory_name = 'D:\results\matlab Codes\Population Analysis\The neural decoding toolbox\ndt.1.0.4_exported';  % put name of path to the Neural Decoding Toolbox
raster_data_directory_name = 'D:\CQ\Matlab codes\Population Analysis\The neural decoding toolbox\ndt.1.0.4_exported\Zhang_Desimone_7objects_raster_data/';   % put name of path to the raster data

cd('D:\CQ\Matlab codes\Population Analysis\The neural decoding toolbox\ndt.1.0.4_exported/')
...
for i = 0:60% go through each repetition number for each stimulus
...
% use 20 cross-validation splits (which means that 19 examples of each object are used for training...
% and 1 example of each object is used for testing)
num_cv_splits = 15; 
...
% usually more than 2 resample runs are used to get more accurate results, but to save time we are using a small number here
the_cross_validator.num_resample_runs = 2; 

% other useful options:   

% can greatly speed up the run-time of the analysis by not creating a full TCT matrix ...
%(i.e., only trainging and testing the classifier on the same time bin)
...
DECODING_RESULTS = the_cross_validator.run_cv_decoding; %Method(run_cv_decoding) in "standard_resample_CV(class)"
%DECODING_RESULTS.ZERO_ONE_LOSS_RESULTS.decoding_results=[num_resample_runs x num_CV_splits x num_training_times x num_test_times]
%num_test_times: test times for each time bin, default=1, if performing
%temporal lampsed decoding, num_test_times=num_training_times;
%%  11.  Save the results
...
if PlotFigure==1
    ...
    plot_obj.plot_results;   % actually plot_results
