...
toolbox_directory_name = 'D:\CQ\Matlab codes\Population Analysis\The neural decoding toolbox\ndt.1.0.4_exported/';  
...
for i = 0:60%find the neuron index with specific trial number for each stimulus condition
...
num_cv_splits = 20; %the trial number for each stimulus condition used to perform the decoding analysis
...
%test times
the_cross_validator.num_resample_runs = 50;  % usually more than 2 resample runs are used to get more accurate results
%, but to save time we are using a small number here
